...
import type { LessonMapItem } from '../services/lessons'
import { APP_THEME } from '../constants/theme'

// ─── PALETTE ──────────────────────────────────────────────────────────────────
const DG       = APP_THEME.colors.darkGreen
const LG       = APP_THEME.colors.lightGreen
const BG       = APP_THEME.colors.background
const PATH_C   = APP_THEME.colors.path
        ...
        <Animated.View style={[s.panel, { transform: [{ translateY: slideY }] }]}>
          <Pressable>
            ...
            <Text style={s.panelStatus}>
              {prog.status === 'locked' ? 'Locked (demo preview enabled)' : isCompleted ? 'Completed ✓' : 'In Progress'}
            </Text>
...
export default function LessonMap({ lessons }: LessonMapProps) {
  ...
  return (
    <View style={s.root}>
      ...
      {/* ── TAB BAR ─────────────────────────────────────────────── */}
      {/* Wire these up to your navigation system */}
      <View style={s.tabbar}>
        {[
